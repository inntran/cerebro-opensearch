...
object IndexSettingsControllerSpec extends MockedServices {
  ...
  def get = {
    ...
    client.getIndexSettingsFlat("foo", OpenSearchServer(Host("somehost", None))) returns Future.successful(Success(200, expectedResponse))
  ...
  def update = {
    ...
    val expectedResponse = Json.parse(
      """
        ...
        |}
      """.stripMargin)
    client.updateIndexSettings("foo", body, OpenSearchServer(Host("somehost", None))) returns Future.successful(Success(200, expectedResponse))
