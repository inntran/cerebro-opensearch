...
object ClusterSettingsControllerSpec extends MockedServices {
  ...
  def getSettings = {
    ...
    client.getClusterSettings(OpenSearchServer(Host("somehost", None))) returns Future.successful(Success(200, expectedResponse))
  ...
  def updateSettings = {
    ...
    val expectedResponse = Json.parse(
      """
        ...
        |}
      """.stripMargin)
    client.saveClusterSettings(body, OpenSearchServer(Host("somehost", None))) returns Future.successful(Success(200, expectedResponse))
