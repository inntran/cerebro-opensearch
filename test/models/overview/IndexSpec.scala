...
object IndexSpec extends Specification {
  ...
  private def sortedShards(json: JsValue): JsValue = {
    val shards = (json \ "shards").as[JsObject]
    val normalized = JsObject(shards.fields.map { case (node, entries) =>
      node -> JsArray(entries.as[JsArray].value.sortBy { shard =>
        ((shard \ "shard").as[Int], (shard \ "state").as[String], (shard \ "primary").as[Boolean])
      })
    })
    json.as[JsObject] + ("shards" -> normalized)
  }
  ...
  def healthy = {
    ...
    sortedShards(index) mustEqual sortedShards(expected)
  }
